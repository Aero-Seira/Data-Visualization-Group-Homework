...
function drawChart(){
  ...
  const w=560,barH=28,gap=7,top=8,bot=20;
  ...
  // names
  svg.selectAll("text.nl").data(data).join("text").attr("class","nl")
    .attr("x",cx-6).attr("y",(d,i)=>top+i*(barH+gap)+barH/2+4)
    .attr("text-anchor","end").style("fill","#333").style("font-size","13px").style("font-weight","700")
    .text(d=>d.name_cn);
