...
│
├── discipline_correlation.py           # → discipline_graph.json (可选AI分析)
├── discipline_graph.json               # 学科关联图数据
├── world-50m.json                      # TopoJSON 世界地图
├── .geo_cache.json                     # 过滤后的 GeoJSON 缓存
│
└── 数据/                               # Excel 原始数据
    ├── 1.中东欧群体发文数量/
    ├── 2.各国发文量（135-125）/
...
```
Excel 数据  ──▶  Python 脚本 (pandas 读取 + 处理)
                     │
                     ├── 常量与工具来自 common.py
                     ├── 数据嵌入 HTML 模板（JSON 占位符）
                     ├── D3.js v7 渲染
                     │
                     ▼
                自包含 .html（浏览器直接打开）
